...
import { processIngredientLine } from '@/utils/recipeConversions'; // <--- הנה הייבוא החדש שלנו!
...
export default function RecipePage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 text-center">
        <div>
          ...
          <button onClick={() => router.replace('/AllRecipes')} className="px-6 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-full font-medium transition-colors shadow-sm">
            חזור לכל המתכונים
          </button>
        </div>
      </div>
  ...
  const nextImage = (e) => { e.preventDefault(); e.stopPropagation(); setCurrentImageIndex((prev) => (prev + 1) % allImages.length); };
  const prevImage = (e) => { e.preventDefault(); e.stopPropagation(); setCurrentImageIndex((prev) => (prev - 1 + allImages.length) % allImages.length); };
    ...
    <div className="min-h-screen bg-[#FAFAFA] pb-24 text-right" dir="rtl">
      ...
      {isImageOpen && (
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4" onClick={() => setIsImageOpen(false)}>
          <button className="absolute top-6 right-6 p-2 bg-white/10 hover:bg-white/30 text-white rounded-full transition-colors cursor-pointer z-[110]" onClick={(e) => { e.stopPropagation(); setIsImageOpen(false); }}>
            <X className="w-8 h-8" />
          </button>
          {allImages.length > 1 && (
            <>
              <div className="absolute inset-0 flex items-center justify-between px-4 sm:px-10 pointer-events-none z-[105]">
                 <button onClick={prevImage} className="bg-white/10 hover:bg-white/30 text-white p-3 sm:p-4 rounded-full pointer-events-auto transition-transform hover:scale-110"><ChevronRight className="w-8 h-8 sm:w-12 sm:h-12" /></button>
                 <button onClick={nextImage} className="bg-white/10 hover:bg-white/30 text-white p-3 sm:p-4 rounded-full pointer-events-auto transition-transform hover:scale-110"><ChevronLeft className="w-8 h-8 sm:w-12 sm:h-12" /></button>
              </div>
          ...
          <img src={allImages.length > 0 ? optimizeImage(getDisplayImage(allImages[currentImageIndex]), 1200, null, 'limit') : defaultImage} alt={`תמונה ${currentImageIndex + 1}`} className="max-w-full max-h-[90vh] object-contain rounded-md relative z-[100]" onClick={(e) => e.stopPropagation()} />
        </div>
      )}
      ...
      <div className="relative h-72 sm:h-96 group overflow-hidden bg-gray-100">
        <div className="absolute inset-0 cursor-zoom-in z-10" onClick={() => setIsImageOpen(true)}>
          <img src={allImages.length > 0 ? optimizeImage(getDisplayImage(allImages[currentImageIndex]), 800, 600) : defaultImage} alt={recipe.name} className="w-full h-full object-cover transition-transform duration-500" onError={(e) => (e.target.src = defaultImage)} />
        ...
        {allImages.length > 1 && (
           <div className="absolute inset-0 pointer-events-none z-20">
             ...
             <button onClick={prevImage} className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-auto bg-white/90 hover:bg-white text-amber-600 p-3 rounded-full shadow-xl transition-transform hover:scale-110 active:scale-95 border border-amber-100"><ChevronRight className="w-6 h-6 sm:w-8 sm:h-8" /></button>
             <button onClick={nextImage} className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-auto bg-white/90 hover:bg-white text-amber-600 p-3 rounded-full shadow-xl transition-transform hover:scale-110 active:scale-95 border border-amber-100"><ChevronLeft className="w-6 h-6 sm:w-8 sm:h-8" /></button>
      ...
      <div className="max-w-lg mx-auto px-4 py-8 space-y-8">
        {isChecklistMode && (
          <div className="bg-amber-100 text-amber-800 p-3 rounded-xl text-sm font-medium flex items-center justify-center gap-2 animate-in slide-in-from-top-2"><ListChecks className="w-5 h-5" /> מצב צ'קליסט פעיל - לחצי על שורות כדי לסמן אותן</div>
        )}

        {recipe.tags && recipe.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 justify-start">
            {recipe.tags.map((tag, i) => (<Badge key={i} className="bg-amber-100 text-amber-800 hover:bg-amber-200 border-amber-200 px-3 py-1 text-sm">{tag}</Badge>))}
          </div>
        )}
            ...
            <div className="flex flex-wrap gap-3 pb-4">
              {allImages.map((imgUrl, i) => (
                <div key={i} className={`w-32 h-32 shrink-0 rounded-xl overflow-hidden shadow-sm cursor-pointer group relative border-2 transition-colors ${i === currentImageIndex ? 'border-amber-400' : 'border-gray-100'}`} onClick={() => { setCurrentImageIndex(i); setIsImageOpen(true); }}>
                  <img src={optimizeImage(getDisplayImage(imgUrl), 200, 200)} alt={`${recipe.name} - ${i + 1}`} loading="lazy" className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" onError={(e) => (e.target.src = defaultImage)} />
          ...
          <section className="bg-white border border-gray-100 rounded-3xl p-5 sm:p-6 shadow-sm transition-all">
            ...
            <div className="flex flex-col gap-4 border-b border-gray-100 pb-5 mb-5">
              <h2 className="text-xl font-bold text-gray-800 flex items-center gap-3">
                <div className="w-8 h-8 bg-amber-100 rounded-lg flex items-center justify-center text-amber-600 shrink-0"><BookOpen className="w-5 h-5" /></div>
                מרכיבים
              </h2>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <div className="flex items-center gap-1 bg-amber-50/50 p-1.5 rounded-xl border border-amber-100 w-full sm:w-auto">
                   <button onClick={() => setConvertMode('original')} className={`flex-1 sm:flex-none px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${convertMode === 'original' ? 'bg-amber-500 text-white shadow-sm' : 'text-amber-700 hover:bg-amber-100'}`}>
                     מקורי
                   </button>
                   <button onClick={() => setConvertMode('grams')} className={`flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${convertMode === 'grams' ? 'bg-amber-500 text-white shadow-sm' : 'text-amber-700 hover:bg-amber-100'}`}>
                     <Scale className="w-3.5 h-3.5" /> לגרמים
                   </button>
                   <button onClick={() => setConvertMode('volume')} className={`flex-1 sm:flex-none flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${convertMode === 'volume' ? 'bg-amber-500 text-white shadow-sm' : 'text-amber-700 hover:bg-amber-100'}`}>
                     <Droplet className="w-3.5 h-3.5" /> לכוסות
                   </button>
                </div>
              </div>
            </div>
                ...
                if (isHeading) {
                  return (
                    <div key={`heading-${i}`} className="col-span-full mt-6 mb-2 first:mt-0">
                      <h3 className="text-lg font-bold text-amber-700 bg-amber-50/50 inline-block px-3 py-1 rounded-lg border border-amber-100/50">
                        {line}
                      </h3>
                    </div>
                  );
                ...
                return (
                  <li key={i} onClick={() => isChecklistMode && toggleIngredient(i)} className={`flex items-start gap-3 transition-all duration-200 ${isChecklistMode ? 'cursor-pointer hover:bg-gray-50 p-2 -mx-2 rounded-xl' : ''}`}>
                    ...
                    <span className={`text-right leading-relaxed font-medium transition-all duration-200 flex items-center flex-wrap ${isChecklistMode && isChecked ? 'text-gray-400 line-through' : 'text-gray-800'}`}>
                      {needsWarning && (
                        <span 
                          title="שים לב, ייתכן שהכמות בשורה זו לא הוכפלה אוטומטית (וודא ידנית)" 
                          className="inline-flex items-center ml-2 text-orange-400/80 cursor-help"
                        >
                ...
                return recipe.method.split("\n").map(l => l.trim()).filter(Boolean).map((step, i) => {
                  ...
                  return (
                    <div key={i} onClick={() => isChecklistMode && toggleStep(i)} className={`flex gap-4 group transition-all duration-200 ${isChecklistMode ? 'cursor-pointer hover:bg-gray-50 p-3 -mx-3 rounded-xl border border-transparent hover:border-gray-100' : ''} ${isChecked && isChecklistMode ? 'opacity-50' : ''}`}>
                      ...
                      <p className={`text-gray-700 pt-1 leading-relaxed text-right text-lg transition-all duration-200 ${isChecklistMode && isChecked ? 'line-through' : ''}`}>{step}</p>
                    </div>
                  );
                });
              })()}
